            ...
            for clustering in clustering_list:
                ...
                if os.path.exists(mod_path+mod_file):
                    ...
                    for version in version_list_nice_names:
                        ...
                        vmin = -max([abs(e_adim.min()), abs(e_adim.max())])
                        vmax = max([abs(e_adim.min()), abs(e_adim.max())])

                        fig0 = ax.imshow(e_adim.values.reshape(365, 24), aspect='auto', cmap='viridis', vmin=vmin, vmax=vmax)

                        fig.colorbar(fig0, label='CF error')

                        ax.set_xlabel('Hour of the Day')
                        ax.set_ylabel('Day of the Year')

                        title = f'{carrier}, {year}, {version}, {domain}, {clustering}'
                        ax.set_title(title)


                        #### Export figure
                        fig_path = 'figs/evaluation/error_t/'
                        fig_name = f'error_t_{domain}_{clustering}_{carrier}_{year}_{version}.jpg'
